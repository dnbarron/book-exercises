...
avo <- read.csv("data/avocado.csv", stringsAsFactors = FALSE)
...
avo <- mutate(avo, Date = as.Date(Date))
...
by_type <- avo %>% group_by(Date, type) %>%
...
by_type_wide <- by_type %>%
  pivot_wider(names_from =  type, values_from = volume)
