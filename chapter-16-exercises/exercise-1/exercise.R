...
g + aes(carat, price, shape = cut) + 
  geom_point()

# Draw a scatter plot for `diamonds_sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*

g + aes(cut, carat, size = price) +
  geom_point()

# Try coloring the above plot based on the diamond's price!
g + aes(cut, carat, size = price, colour = price) +
  geom_point()


# Draw a line plot (with line geometry) for `diamonds_sample`. The x-position 
# should be mapped to carat, y-position to price, and color to cut.
g + aes(carat, price, colour = cut) +
  geom_line()
...
g + aes(cut, price) + geom_col()

# Add an aesthetic property that will _fill_ each bar geometry based on the 
# `clarity` of the diamonds. 
# What kind of chart do you get?
g + aes(cut, price, fill = clarity) + 
  geom_col()
...
clarity_summary <- diamonds %>%
  ...
  summarise(mean_price = mean(price, na.rm = TRUE),
            n = n(),
            std_error = sd(price, na.rm = TRUE) / sqrt(n))

clarity_summary %>% ggplot(aes(x = clarity, y = mean_price)) +
  geom_col() +
  geom_errorbar(aes(ymax = mean_price + std_error, ymin = mean_price - std_error), colour = 'red')
