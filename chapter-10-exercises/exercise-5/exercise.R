...
most_popular_in_year <- function(year){
  names_year <- names[names$year == year, c("name", "prop")]
...
number_in_millions <- function(name, year){
  names[names$name == name & names$year == year, "prop"] * 1e6
}

# How many babies out of 1 million had the name 'Laura' in 1995?
number_in_millions('Laura', 1995)

# How many babies out of 1 million had your name in the year you were born?
number_in_millions('David', 1995)
