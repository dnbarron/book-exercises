...
setwd("C:/Users/dnbar/Documents/book-exercises/chapter-11-exercises/exercise-8")
...
# 2004-2014?
pulitzer %>% arrange(desc(Pulitzer.Prize.Winners.and.Finalists..2004.2014)) %>%
  select(Newspaper) %>%
  head()
...
library(stringr)
ch <- pulitzer$Change.in.Daily.Circulation..2004.2013
as.numeric(str_sub(ch, 1, -2))

pulitzer %>% filter(Pulitzer.Prize.Winners.and.Finalists..2004.2014 > 4) %>%
  mutate(circ_change = as.numeric(str_sub(Change.in.Daily.Circulation..2004.2013, 1, -2))) %>%
  select(Newspaper, circ_change) %>%
  filter(circ_change == min(circ_change))
...
pulitzer %>%
  mutate(circ_change = as.numeric(str_sub(Change.in.Daily.Circulation..2004.2013, 1, -2))) %>%
  summarise(cor(circ_change, Pulitzer.Prize.Change))
